...
\begin{theorem}[Collapse-3 / Constancy]
If the trajectory starts from a universal configuration where all states are equivalent, then any stable classifier must be constant:
\[
\mathrm{Universal}(d_0) \land \mathrm{StableK}(d_0, K) \implies \mathrm{Constant}(K)
\]
\end{theorem}

\begin{proof}
Immediate from definitions. $\mathrm{StableK}$ gives $\mathrm{Respects}(\mathrm{traj}(d_0, 0), K)$. Since $\mathrm{traj}(d_0, 0) = d_0$ and $\mathrm{Universal}(d_0)$ gives $x \sim_{d_0} y$ for all $x, y$, we have $K(x) = K(y)$ for all $x, y$.
\end{proof}
...
\begin{itemize}
\item \texttt{ForcingT1.agda}: ComposableObs, witnessComp, T1-potential-split, StabilizesRobust
\item \texttt{ForcingT2.agda}: Iter, encode/decode isomorphism, iter-induction (\textbf{fully proven})
\item \texttt{ForcingT3.agda}: Universal, T3-constancy (\textbf{fully proven}), T3-connected (\textbf{fully proven})
\end{itemize}
